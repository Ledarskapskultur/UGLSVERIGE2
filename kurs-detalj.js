(function(){
  ...
  var kurser=window.UGL_RADER.split('\n').map(function(rad){
    ...
    return {start:start,slut:slut,vecka:+d[1],anlaggning:d[2],ort:d[3],region:REGIONER[d[3]]||d[3],
      ...
      egenBild:!!(ANL[d[2]]&&ANL[d[2]].bild),anlText:(ANL[d[2]]&&ANL[d[2]].text)||'',
  ...
  document.getElementById('k-bild').alt=k.egenBild?k.anlaggning+', '+k.ort:'Deltagare under en UGL-vecka';
  document.getElementById('k-bildtext').textContent=k.egenBild
    ? k.anlaggning+', '+k.ort
    : 'Bilden är från en UGL-vecka och visar inte '+k.anlaggning+'.';
  if(k.anlText){
    var pl=document.createElement('p');
    pl.className='anl-beskrivning';
    pl.textContent=k.anlText;
    document.querySelector('#oversikt').appendChild(pl);
  }
